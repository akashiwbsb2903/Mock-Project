...
def build_context_snippets(
    ...
    *,
    ...
    # If no relevant snippets, fallback: take top N by recency (even if score is 0)
    if tmp.empty:
        tmp = pd.DataFrame({
            "score": scores,
            "title": title,
    ...
    # If the query is about trends, prioritize reddit/blogs first
    trend_words = {"trend", "trending", "trends"}
    ...
    for src in sources_to_include:
        src_rows = tmp[tmp["source"].str.lower() == src]
        if not src_rows.empty:
            row = src_rows.iloc[0]
            snippet_text = _make_excerpt(row.get("text") or "", q_toks, min_chars, max_chars)
            if snippet_text and not any(is_similar(snippet_text, t) for t in seen_texts):
                dt = row.get("date")
                dt_str = None
                if pd.notna(dt):
                    try:
    ...
    # Now fill up to topn with most relevant remaining snippets, avoiding duplicates
