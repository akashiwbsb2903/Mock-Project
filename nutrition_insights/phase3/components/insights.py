...
def render_buzz(df: pd.DataFrame, meta=None) -> None:
    ...
    # Prepare context for LLM: use top buzz posts with evidence, but do NOT include refs inline
    buzz_evidence = []
    references = []
    seen_refs = set()
    for _, r in protein_sdf.iterrows():
        ...
        if not title and not text:
            continue
        buzz_evidence.append(f"- **{title}**: {text}")
        if url and title and url not in seen_refs:
    ...
    prompt = (
        ...
        "Use as many unique references as possible from the evidence, but do not repeat references in the bullet points.\n\n"
        f"{buzz_context}"
    )
...
def render_insights():
    ...
    summary = chat_completion(prompt, model=openai_model, timeout=180)
    findings, references = summary, ""
    if "REFERENCES:" in summary:
        findings, references = summary.split("REFERENCES:", 1)
    st.markdown("""
